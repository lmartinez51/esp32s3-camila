...
- 🛠️ **Modular Robot HAL & Multi-Protocol Drivers** — unified hardware abstraction layer (`hal/`), driver ecosystem (`drivers/`) supporting BLE (Elegoo BT16, Generic NUS), IR (RMT NEC, RC5, Sony), and WiFi (Pan-Tilt, Robotic Arm, HTTP/TCP), backed by a persistent device registry (`registry/`) and dynamic WebRTC tool adapter (`adapters/`).
