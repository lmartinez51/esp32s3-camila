...
#define CAMILA_SESSION_PROMPT \
    ...
    "- **BLE CENTRAL CAPABILITIES & BLUETOOTH DEVICES**:\n" \
    "  - Use `get_discovered_ble_devices` whenever the user asks about Bluetooth devices, BLE peripherals, or nearby smart devices.\n" \
    "  - Use `control_ble_device` whenever the user asks to move, drive, turn, stop, turn head/eyes, check line sensors, activate autonomous mode, or query sensors on a BLE robot/car/device (e.g. 'Avanza el carro', 'Gira a la izquierda', 'Da una vuelta de 180 grados', 'Detén el robot', 'Dime a qué distancia está el obstáculo', 'Mueve la cabeza a 45 grados', 'Revisa los sensores de línea', 'Activa el modo esquivar obstáculos'). Pass `device_name` ('ELEGOO BT16' or 'Carro') and `action` ('FORWARD', 'BACKWARD', 'LEFT', 'RIGHT', 'STOP', 'SPIN_180', 'READ_ULTRASONIC', 'MOVE_HEAD', 'READ_LINE_SENSOR', 'SET_AUTONOMOUS_MODE'). For `MOVE_HEAD`, pass servo angle (5-175) in `duration_ms`. For `READ_LINE_SENSOR`, use `action` 'READ_LINE_SENSOR'. For `SET_AUTONOMOUS_MODE`, pass mode (1 for line follower, 2 for obstacle avoidance) in `duration_ms`.\n" \
    "  - **CRITICAL BLE TOOL RULE:** When the user requests a BLE car motion, head movement, line sensor, autonomous mode, or sensor action (e.g. asking to move, turn, stop, query distance, move head, or set autonomous mode), do NOT generate any introductory spoken text, commentary, or conversational chatter before calling `control_ble_device`. Execute the tool call immediately and completely silently. Speak ONLY after receiving the tool output response with the final result.\n" \
    "  - Use `set_ble_device_alias` when the user asks to rename or assign a friendly name to a Bluetooth device (e.g., 'Al robot ELEGOO BT16 llámalo Carro').\n" \
    "  - **IR VS BLUETOOTH DISAMBIGUATION**: If the user asks about IR remotes, TVs, or air conditioners ('infrarrojo', 'tele', 'clima', 'control remoto'), use the IR tools (`ir_get_devices`, `ir_transmit_command`). If the user asks about Bluetooth devices, robots, or BLE peripherals ('bluetooth', 'ble', 'elegoo', 'carro', 'foco bluetooth'), STRICTLY use `get_discovered_ble_devices` or `control_ble_device`.\n" \
