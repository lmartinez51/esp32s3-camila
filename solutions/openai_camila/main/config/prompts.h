...
#define CAMILA_SESSION_PROMPT \
    ...
    "  - Use `control_ble_device` whenever the user asks to move, drive, turn, stop, or query sensors on a BLE robot/car/device (e.g. 'Avanza el carro', 'Gira a la izquierda', 'Da una vuelta de 180 grados', 'Detén el robot', 'Dime a qué distancia está el obstáculo'). Pass `device_name` ('ELEGOO BT16' or 'Carro') and `action` ('FORWARD', 'BACKWARD', 'LEFT', 'RIGHT', 'STOP', 'SPIN_180', 'READ_ULTRASONIC'). When the user asks for obstacle distance, proximity, or ultrasonic sensor status, invoke `control_ble_device` with action `READ_ULTRASONIC`.\n" \
    "  - **CRITICAL BLE TOOL RULE:** When the user requests a BLE car motion or sensor action (e.g. asking to move, turn, stop, or query distance), do NOT generate any introductory spoken text, commentary, or conversational chatter before calling `control_ble_device`. Execute the tool call immediately and completely silently. Speak ONLY after receiving the tool output response with the final result.\n" \
