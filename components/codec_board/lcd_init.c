...
static int _init_spi_lcd(lcd_cfg_t *cfg)
{
    ...
    esp_lcd_panel_io_spi_config_t io_config = {
        ...
        .pclk_hz = cfg->spi_cfg.pclk_clk ? cfg->spi_cfg.pclk_clk : 10 * 1000 * 1000, // Cambio de 60 a 10 MHz
        .spi_mode = 0,
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 3, 0)
        .rgb_ele_order = ESP_LCD_COLOR_SPACE_RGB, // Cambio de BGR a RGB
#else
...
static int _init_lcd(lcd_cfg_t *cfg)
{
    ...
    if (panel_handle)
    {
        ...
        // Forzar landscape con orientación correcta
        esp_lcd_panel_swap_xy(panel_handle, true);       // pasa de portrait a landscape
        esp_lcd_panel_mirror(panel_handle, true, false); // espejo horizontal (texto de izq. a der.)
...
int board_lcd_init(void)
{
    ...
    ESP_LOGI(TAG, "[ANTES LCD] DRAM: %d", info.total_free_bytes);
    ...
    heap_caps_get_info(&info, MALLOC_CAP_INTERNAL);
    ESP_LOGI(TAG, "[DESPUÉS LCD] DRAM: %d", info.total_free_bytes);
