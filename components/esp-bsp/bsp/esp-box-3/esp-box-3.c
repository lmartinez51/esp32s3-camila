...
static const bsp_button_config_t bsp_button_config[BSP_BUTTON_NUM] = {
    ...
    {.type = BSP_BUTTON_TYPE_GPIO, .cfg.gpio = {
                                       .gpio_num = BSP_BUTTON_MUTE_IO,
                                       .active_level = 0,
                                   }},
    {.type = BSP_BUTTON_TYPE_CUSTOM, .cfg.custom = {.enable_power_save = false, .get_key_level = bsp_get_main_button, .enter_power_save = NULL, .del = NULL}}};
...
lv_display_t *bsp_display_start(void)
{
    bsp_display_cfg_t cfg = {
        ...
        .flags = {
            .buff_dma = false,
            .buff_spiram = true, // Use PSRAM for LVGL buffers
        }};
...
esp_err_t bsp_iot_button_create(button_handle_t btn_array[], int *btn_cnt, int btn_array_size)
{
    ...
    for (int i = 0; i < BSP_BUTTON_NUM; i++)
    {
        if (bsp_button_config[i].type == BSP_BUTTON_TYPE_CUSTOM)
        {
            if (tp == NULL)
            {
                // PARCHE. Si no se ha inicializado el touch, no podemos crear el botón
                esp_err_t tr = bsp_touch_new(NULL, &tp);
                if (tr == ESP_ERR_NOT_FOUND)
                {
                    ESP_LOGW(TAG, "Touch no encontrado, saltando init de touch");
                }
                else
                {
                    // en cualquier otro caso de error, sí abortamos
                    BSP_ERROR_CHECK_RETURN_ERR(tr);
                }
            }
